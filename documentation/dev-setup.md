...
The intended end state is **option E** — every layer becomes a self-contained, installable package. Each layer dir gets its own `package.json` listing only the externals that layer's code actually imports, plus its own `node_modules` populated by `bun install` inside that dir. Build-time deps like `@nuxt/kit` go into each layer's `dependencies` (not `peerDependencies`) so giget's `install: true` brings them down in production. The host's `package.json` slims to just the host-specific deps: `nuxt`, eslint tooling, the icon set. The `layers/node_modules` symlink goes away — each layer resolves its own deps from its own neighborhood, regardless of where on disk the layer lives. Dev and prod become structurally identical: in both, a layer is a folder containing `package.json` + `node_modules` + source. Cost is roughly 8× duplication of common deps on dev disk and the need to keep dep versions in sync across 9 package.jsons; both are tolerable now and can be addressed later with a bun-workspaces overlay if duplication starts to hurt. The trigger for doing the migration is when a layer needs to move to its own git repo or be published to npm — at that point the per-layer `package.json` is required anyway, so the rest follows.
