...
Each layer is a named workspace package — `@nuxtinator/<id>` with `exports: { ".": "./nuxt.config.ts" }`. [host/nuxt.config.ts](../host/nuxt.config.ts) lists each in `extends:` by package name; Nuxt resolves the name through standard node module resolution against `node_modules/`:
...
3. Add `"@nuxtinator/<id>": "workspace:*"` to `host/package.json` `dependencies`.
4. Add `layer('@nuxtinator/<id>')` to `extends:` in [host/nuxt.config.ts](../host/nuxt.config.ts).
5. `bun install` from root, then `cd host && bun run dev`.
...
- Build context must include the workspace root so `node_modules/@nuxtinator/*` resolves. `bun install && cd host && bun run build` from the repo root.
- Run: standard Nuxt output under `host/.output/` — see Nuxt deployment docs for your target.
