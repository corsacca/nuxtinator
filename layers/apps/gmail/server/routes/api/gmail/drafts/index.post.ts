// POST /api/gmail/drafts — start a draft (new, reply, reply-all, forward).
import { z } from 'zod'
...
const Body = z.object({
  ...
  mode: z.enum(GMAIL_DRAFT_MODES).default('new'),
...
export default defineEventHandler(async (event) => {
  return await withOrgPermission(event, { appId: 'gmail' }, 'gmail.access', async (tx, ctx) => {
    ...
    try {
      const row = await gmailCreateDraft(tx, ctx.userId, {
        ...parsed.data,
