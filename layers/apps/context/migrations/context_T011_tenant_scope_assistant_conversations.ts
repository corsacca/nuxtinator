...
// `context_assistant_conversations` directly: a workspace-wide conversation
// has no portfolio to cascade through, so unlike the other child tables it
// can't inherit isolation from `context_portfolios`. Messages cascade via FK
// to the conversation and are protected transitively.
...
export async function up(db: Kysely<unknown>): Promise<void> {
  await enableTenantScoping(db, 'context_assistant_conversations')
}

export async function down(db: Kysely<unknown>): Promise<void> {
  await disableTenantScoping(db, 'context_assistant_conversations')
}
