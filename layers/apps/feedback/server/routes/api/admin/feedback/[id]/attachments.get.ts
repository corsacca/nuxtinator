...
export default defineEventHandler(async (event) => {
  ...
  // See PATCH /api/admin/feedback/:id for the rationale: `cards` is RLS-
  // protected, so we need the active-org GUC set to read it. `withRecord-
  // OrgContext` resolves the card's owning org via adminDb and primes the GUC
  // before running our reads.
